...
var japanPrefectures = ["Kagawa","Wakayama","Nagasaki","Nara","Nagoya"];
...
var bigBox = {};
bigBox.size = Math.floor(Math.random()*12);
bigBox.color = "red";
bigBox.contents = [];
...
function displayArray(array) {
  for(var i = 0;i<array.length;i++) {
    console.log("The location at " + i + " is: " + array[i] + ".");
  }
}

displayArray(japanPrefectures);
...
function addUp(finNum) {
  var sum = 0;
  for(var i = 1;i<=finNum;i++) {
    sum += i;
  }
  console.log(sum);
}
...
console.log(firstReverse("boolean"));
...
function letterCap(str) {
  ...
  newString = newString.concat(str.substring(1));
  console.log(newString);
}
